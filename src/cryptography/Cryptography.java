...
public interface Cryptography {
	...
	/**
	 * validates the MAC for message, comparing with expectedMac, using the outerMac configurations
	...
	/**
	 * computes an integrity proof, either a MAC or an HASH, depending on the underlying implementation
	 ...
	 * @return an array of byte[], where the first corresponds to the message and the second to the MAC
	 * */
	public byte[][] splitOuterMac(byte[] messageWithMac);

	/**
	 * splits the integrity proof from the message
	 * @return an array of byte[], where the first corresponds to the message and the second to the integrity proof
	 * */
